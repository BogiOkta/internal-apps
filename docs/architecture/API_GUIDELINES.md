...
| `GET /api/v1/organization/employees` | `search`, `employeeNumber`, `name`, `departmentPublicId`, `email`, `status`, `sort` | Returns employees with department display data; filters combine server-side and use allowlisted values. Without `sort`, Active employees precede inactive employees and employee numbers are natural ascending within each group. |
